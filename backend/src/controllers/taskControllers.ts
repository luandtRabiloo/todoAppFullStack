...
export const getAllTask = async (req: Request, res: Response) => {
    ...
    try {
        const result = await Task.aggregate([
            ...
            {
                $facet: {
                    tasks: [{ $sort: { createdAt: -1 } }, { $skip: skip }, { $limit: pageSize }],
...
export const createTask = async (req: Request<{}, {}, TTaskBody>, res: Response) => {
    try {
        ...
        if (!newTask) {
            res.status(404).json({ message: 'Nhiệm vụ không tồn tai' });
        }
        console.log({ newTask });
...
export const editTask = async (req: Request<{ id: string }, {}, TTaskBody>, res: Response) => {
    try {
